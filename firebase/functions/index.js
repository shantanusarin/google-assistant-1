...
var serviceAccount = require("./config/triggerbot-c85a9-firebase-adminsdk-wr8pa-9b89a6af4e.json");

admin.initializeApp({
  credential: admin.credential.cert(serviceAccount),
  databaseURL: "https://triggerbot-c85a9.firebaseio.com"
});
...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
  ...
  const actionHandlers = {
    ...
    'input.welcome': () => {
        console.log('Hi sajnkajxnkj shantanu');
      // Use the Actions on Google lib to respond to Google requests; for other requests use JSON
      if (requestSource === googleAssistantRequest) {
        sendGoogleResponse('Hi shantanu!'); // Send simple response to user
      } else {
    ...
    'input.vote': () => {
        console.log(parameters);
         ...
         let responseText='';
     let singer=app.getArgument('Singer');
     if(singer!==null){
      ...
      // Use the Actions on Google lib to respond to Google requests; for other requests use JSON
      if (requestSource === googleAssistantRequest) {
        sendGoogleResponse('Hi finally voting ...BUT from Firebase externally !'); // Send simple response to user
      } else {
        sendResponse('Hello, Welcome to my Dialogflow agent!'); // Send simple response to user
      }
    },
